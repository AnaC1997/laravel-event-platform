            ...
            <form action="{{ route('admin.events.update', $event->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                        name="name" placeholder="Inserisci il nome dell'evento" value="{{ old('name') ?? $event->name }}">
                        @error('name')
                ...
                <div class="mb-3">
                    ...
                    <input type="date" class="form-control @error('date') is-invalid @enderror" id="date"
                        name="date" placeholder="Inserisci la data dell'evento" value="{{ old('date') ?? $event->date }}">
                    @error('date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('available_tickets') is-invalid @enderror"
                        ...
                        placeholder="Inserisci la quantita di tickets disponibili" value="{{ old('available_tickets') ?? $event->available_tickets }}">
